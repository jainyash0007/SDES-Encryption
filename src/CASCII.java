...
public class CASCII {
  ...
  {
    // make an bitOutput array of bytes that is the converted text
  ...
  {
    // make an bitOutput array of bytes that is the converted text
    char bitOutput[] = new char[cas.length/5];
    ...
    for ( int block = 0; block < cas.length; block += 5 )
    {
      ...
      bitOutput[block/5] = CASCII.Convert(cas, block, block+4);
    }

    // send back the new byte array
    return new String(bitOutput);
  }
